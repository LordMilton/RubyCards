...
Run Enigma Virtual Box and open the project file in `scripts/`. The actual paths that it will try to pull these files from is specific to my machine, you will need to essentially build your own EVB project with the shape that is indicated by the project you opened. Make sure to change the input (The .exe you created with `iexpress`) and output file paths. You can remove any files in the `scripts/` directory (but obviously not `Ruby/`). Please don't commit any of these changes if you choose to save the project file.
